...
class KaplanMeierTorch:
    ...
    def median(self):
        median_time = torch.tensor([float('inf')], device=self.device)
        for i in range(len(self.survival_probabilities)):
            if self.survival_probabilities[i] <= 0.5:
                median_time = self.survival_times[i]
                break
        return median_time.item()
